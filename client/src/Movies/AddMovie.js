...
const AddMovie = ({ getMovieList }) => {
  ...
  const handleOnStarsChange = e => {
    setData({
      ...data,
      stars: { ...data.stars, [e.target.name]: e.target.value },
    })
  }
  ...
  const handleOnSubmit = e => {
    ...
    let starsString = Object.keys(data.stars)
      .map(star => `${data.stars[star]},`)
      .join('')
    axios
    ...
    <form onSubmit={handleOnSubmit}>
      ...
      {stars.map((star, i) => (
        <input
          key={i}
          type="text"
          onChange={handleOnStarsChange}
          name={`star${stars.length}`}
          value={data.stars[`star${stars.length}`]}
        />
      ))}
      <button type="button" onClick={e => handleAddStars(e)}>
        Add more stars
      </button>
